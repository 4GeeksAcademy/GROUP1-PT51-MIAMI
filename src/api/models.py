...
class Saved(db.Model):
    ...
    # THE FOLLOWING 2 COLUMNS ESTABLISHES RELATIONSHIP BETWEEN
    # CARS ON OUR DATABASE AND USERS ON OUR DATABASE
    user = db.relationship(
        "User", backref="saved_cars", foreign_keys=[user_id]
    )
    car = db.relationship(
        "Car", backref="saved_by_users", foreign_keys=[car_id]
    )
    ...
    def serialize(self):
        return {"car": self.car.serialize() if self.car else None}
...
class User(db.Model):
    ...
    saved = relationship(
        "Saved", lazy=True, back_populates="user", overlaps="saved_cars"
    )
...
class Review(db.Model):
    ...
    def __repr__(self):
        return (
            f"<Review {self.id}, User {self.user_name}, Car {self.car_model}"
        )
